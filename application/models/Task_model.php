...
class Task_model extends CI_Model
{
    ...
    function taskListingByUserValid($userId)
    {
       ...
       $this->db->group_by('BaseTbl.affectationId ');
       $query = $this->db->get();
       $result = $query->result();        
       return $result;
   }
  ...
  function AffectationsListing($tacheId)
    {
        $this->db->select('BaseTbl.affectationId , user.name ,BaseTbl.status , user.name parname , user.avatar , BaseTbl.userAffectatedID , BaseTbl.status  ');
        $this->db->from('tbl_affectation as BaseTbl');
        $this->db->join('tbl_users as user', 'user.userId = BaseTbl.userAffectatedID', 'LEFT');
